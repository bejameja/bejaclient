<template>
  <header class="topbar" :class="{ maximized }" :style="topbarTheme">
    ...
    <Tooltip text="Search & quick actions" placement="bottom">
    ...
    <!-- Instance picker -->
    <button class="instance-btn" @click="openWizard">
      {{ instanceLabel }}
    </button>
...
// Reload the gem balance whenever an account becomes active (initial login,
// or switching accounts) — the topbar can mount before login finishes.
...
.instance-btn {
  ...
  &:hover { background: #1a1a1a; border-color: rgba(255, 255, 255, 0.8); }
}
...
.account-pill--guest {
  ...
  &:hover:not(:disabled) { background: #1a1a1a; border-color: rgba(224, 85, 85, 0.8); }
