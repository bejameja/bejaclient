...
.bc-btn {
  ...
  font-weight: 600;
  cursor: pointer;
  transition: background $transition, color $transition, border-color $transition, opacity $transition, transform 100ms $ease-out;
  white-space: nowrap;
  flex-shrink: 0;
  ...
  &.variant-primary {
    ...
    &:hover:not(.disabled) {
      ...
      background-image: $gradient-sheen;
      box-shadow: 0 1px 0 rgba(255, 255, 255, 0.5) inset, 0 4px 16px rgba(0, 0, 0, 0.35);
      transform: translateY(-1px);
    }
    &:active:not(.disabled) { background: $border-strong; transform: translateY(0); }
  }
