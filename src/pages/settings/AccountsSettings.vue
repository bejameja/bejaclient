...
.account-card {
  ...
  &:hover { border-color: $border-strong; transform: translateY(-1px); box-shadow: $shadow-sm; }
  &.selected {
...
.account-avatar {
  width: 40px; height: 40px; overflow: hidden; border-radius: $radius; flex-shrink: 0;
  transition: box-shadow $transition;
  .account-card.selected & { box-shadow: 0 0 0 2px color-mix(in srgb, var(--accent, #{$primary}) 45%, transparent); }
}
...
.selected-badge {
  font-size: 11px; font-weight: 700; color: var(--accent, #{$primary}); padding: 4px $sp-3;
  display: flex; align-items: center; gap: 4px;
  &::before { content: ''; width: 6px; height: 6px; border-radius: 50%; background: currentColor; box-shadow: 0 0 6px currentColor; }
}
.mini-spinner {
